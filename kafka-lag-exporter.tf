  ...
  data = {
    "application.conf" = templatefile(
      "${path.module}/templates/application.conf", {
        username         = confluent_api_key.kafka_lag_exporter_api_key.id
        password         = confluent_api_key.kafka_lag_exporter_api_key.secret
        namespace        = var.metric_exporters_namespace
        bootstrapBrokers = var.bootstrap_server
...
resource "kubernetes_deployment" "lag_exporter_deployment" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name             = kubernetes_service_account.lag_exporter_service_account.metadata.0.name
        ...
        volume {
          name = "config"
          secret {
            secret_name = kubernetes_secret.lag_exporter_config.metadata.0.name
          }
        }
      }
    }
